...
class WebviewERD {
    ...
    setupHtml() {
        const pathVuerd = vscode.Uri.file(path.join(this.extensionPath, "static", "vuerd.min.js"));
...
var ga_cjs = createCommonjsModule(function (module, exports) {
...
var fs = _interopDefault(fs__default);
...
var dns$1 = _interopDefault(dns);
var os$1 = _interopDefault(os);
var zlib$1 = _interopDefault(zlib);
var http2$1 = _interopDefault(http2);
...
const deferToConnect = (socket, fn) => {
    ...
    const onConnect = () => {
        ...
        if (socket instanceof tls$1.TLSSocket && hasSecureConnectListener) {
...
const URLParser = typeof URL === 'undefined' ? url$1.URL : URL;
	...
	createCacheableRequest(request) {
		return (opts, cb) => {
			...
			if (typeof opts === 'string') {
				url$1$1 = normalizeUrlObject(url$1.parse(opts));
				opts = {};
			} else if (opts instanceof url$1.URL) {
				url$1$1 = normalizeUrlObject(url$1.parse(opts.toString()));
				opts = {};
			} else {
				...
				const search = searchParts.length > 0 ?
					`?${searchParts.join('?')}` :
					'';
				url$1$1 = normalizeUrlObject({ ...opts, pathname, search });
			}
			...
			opts = {
				...
				...opts,
				...urlObjectToRequestOptions(url$1$1)
			};
			opts.headers = lowercaseKeys(opts.headers);

			const ee = new events$1();
			const normalizedUrlString = normalizeUrl_1(
				url$1.format(url$1$1),
				{
					stripWWW: false,
...
const {Transform, PassThrough: PassThrough$1} = stream$1;
...
var decompressResponse = response => {
	...
	const decompressStream = isBrotli ? zlib$1.createBrotliDecompress() : zlib$1.createUnzip();
		...
		return new Promise((resolve, reject) => {
			...
			const entry = () => {
				...
				try {
					...
					const session = http2$1.connect(origin, {
...
const {Readable: Readable$1} = stream$1;
...
var resolveAlpn = (options = {}) => new Promise((resolve, reject) => {
	const socket = tls$1.connect(options, () => {
...
const statAsync = util$1.promisify(fs.stat);
exports.default = async (body, headers) => {
    ...
    if (isFormData.default(body)) {
        return util$1.promisify(body.getLength.bind(body))();
    }
...
exports.default = (request, delays, options) => {
    ...
    once(request, 'socket', (socket) => {
        ...
        if (socket.connecting) {
            const hasPath = Boolean(socketPath !== null && socketPath !== void 0 ? socketPath : net$1.isIP((_a = hostname !== null && hostname !== void 0 ? hostname : host) !== null && _a !== void 0 ? _a : '') !== 0);
...
exports.default = (origin, options) => {
    ...
    const url$1$1 = new url$1.URL(origin);
    if (options.path) {
    ...
    for (const key of keys) {
        if (options[key]) {
            url$1$1[key] = options[key].toString();
        }
    }
    return url$1$1;
};
...
class Request extends stream$1.Duplex {
    ...
    static normalizeArguments(url$1$1, options, defaults) {
        var _a, _b, _c, _d;
        const rawOptions = options;
        if (dist.default.object(url$1$1) && !dist.default.urlInstance(url$1$1)) {
            options = { ...defaults, ...url$1$1, ...options };
        }
        else {
            if (url$1$1 && options && options.url) {
            ...
            if (dist.default.urlInstance(options.url)) {
                options.url = new url$1.URL(options.url.toString());
            }
        }
        ...
        if ('searchParams' in options) {
            if (options.searchParams && options.searchParams !== (defaults === null || defaults === void 0 ? void 0 : defaults.searchParams)) {
                if (!dist.default.string(options.searchParams) && !(options.searchParams instanceof url$1.URLSearchParams)) {
                    validateSearchParameters(options.searchParams);
                }
                const searchParameters = new url$1.URLSearchParams(options.searchParams);
        ...
        if (options.url) {
            ...
            Object.defineProperty(options, 'prefixUrl', {
                set: (value) => {
                    const url$1$1 = options.url;
                    if (!url$1$1.href.startsWith(value)) {
                        throw new Error(`Cannot change \`prefixUrl\` from ${prefixUrl} to ${value}: ${url$1$1.href}`);
                    }
                    options.url = new url$1.URL(value + url$1$1.href.slice(prefixUrl.length));
                    prefixUrl = value;
                },
                get: () => prefixUrl
            });
            ...
            if (protocol === 'unix:') {
                protocol = 'http:';
                options.url = new url$1.URL(`http://unix${options.url.pathname}${options.url.search}`);
            }
            // Set search params
            if (options.searchParams) {
        ...
        if (cookieJar) {
            ...
            if (setCookie.length === 4 && getCookieString.length === 0) {
                setCookie = util$1.promisify(setCookie.bind(options.cookieJar));
                getCookieString = util$1.promisify(getCookieString.bind(options.cookieJar));
                options.cookieJar = {
                    setCookie,
                    getCookieString
                };
    ...
    async _finalizeBody() {
        ...
        if (hasPayload) {
            ...
            if (isBody &&
                !(options.body instanceof stream$1.Readable) &&
            ...
            }
            {
                ...
                if (isBody) {
                    ...
                    this[kBody] = (new url$1.URLSearchParams(options.form)).toString();
                }
                else {
                    if (noContentType) {
    ...
    async _onResponse(response) {
        ...
        typedResponse.statusMessage = typedResponse.statusMessage ? typedResponse.statusMessage : http$1.STATUS_CODES[statusCode];
        ...
        if (dist.default.object(options.cookieJar) && rawCookies) {
            let promises = rawCookies.map(async (rawCookie) => options.cookieJar.setCookie(rawCookie, url$1$1.toString()));
        ...
        if (options.followRedirect && response.headers.location && redirectCodes.has(statusCode)) {
            ...
            try {
                ...
                const redirectUrl = new url$1.URL(redirectBuffer, url$1$1);
                const redirectString = redirectUrl.toString();
                decodeURI(redirectString);
                // Redirecting to a different site, clear sensitive data.
                if (redirectUrl.hostname !== url$1$1.hostname) {
    ...
    async _makeRequest() {
        ...
        if (options.http2) {
            ...
            fallbackFn = isHttps ? https$1.request : http$1.request;
        }
...
unwrapExports(core);
var core_1 = core.kIsNormalizedAlready;
var core_2 = core.withoutBody;
var core_3 = core.knownHookEvents;
var core_4 = core.RequestError;
var core_5 = core.MaxRedirectsError;
var core_6 = core.HTTPError;
var core_7 = core.CacheError;
var core_8 = core.UploadError;
var core_9 = core.TimeoutError;
var core_10 = core.ReadError;
var core_11 = core.UnsupportedProtocolError;
...
core.RequestError;
exports.MaxRedirectsError = core.MaxRedirectsError;
exports.CacheError = core.CacheError;
exports.UploadError = core.UploadError;
exports.TimeoutError = core.TimeoutError;
exports.HTTPError = core.HTTPError;
exports.ReadError = core.ReadError;
exports.UnsupportedProtocolError = core.UnsupportedProtocolError;
...
unwrapExports(types);
var types_1 = types.CancelError;
var types_2 = types.RequestError;
var types_3 = types.MaxRedirectsError;
var types_4 = types.CacheError;
var types_5 = types.UploadError;
var types_6 = types.TimeoutError;
var types_7 = types.HTTPError;
var types_8 = types.ReadError;
var types_9 = types.UnsupportedProtocolError;
var types_10 = types.ParseError;
...
if (!core.knownHookEvents.includes('beforeRetry')) {
    core.knownHookEvents.push('beforeRetry', 'afterResponse');
}
...
class PromisableRequest extends core.default {
    ...
    async _beforeError(error) {
        if (!(error instanceof core.RequestError)) {
            error = new core.RequestError(error.message, error, this);
        }
...
var core_2$1 = core$1.knownBodyTypes;
var core_3$1 = core$1.parseBody;
...
exports.PromisableRequest = core$1.default;
...
function asPromise(options) {
    ...
    const promise = new pCancelable((resolve, _reject, onCancel) => {
        const makeRequest = () => {
            ...
            const request = new core$1.default(options.url, options);
            ...
            const onResponse = async (response) => {
                ...
                // Parse body
                try {
                    response.body = core$1.parseBody(response, options.responseType, options.encoding);
                }
                catch (error) {
                    // Fallback to `utf8`
                ...
                try {
                    for (const [index, hook] of options.hooks.afterResponse.entries()) {
                        ...
                        response = await hook(response, async (updatedOptions) => {
                            const typedOptions = core$1.default.normalizeArguments(undefined, {
                                ...updatedOptions,
                                retry: {
                                    calculateDelay: () => 0
                                },
    ...
    const shortcut = (responseType) => {
        const newPromise = (async () => {
            ...
            await promise;
            return core$1.parseBody(globalResponse, responseType, options.encoding);
        })();
...
var asPromise_2 = asPromise_1.PromisableRequest;
...
const getPromiseOrStream = (options) => options.isStream ? new core.default(options.url, options) : asPromise_1.default(options);
...
class Analytics {
    constructor(trackingID, protocolVersion = "1") {
        this.hitType = "event";
...
unwrapExports(ga_cjs);
var ga_cjs_1 = ga_cjs.Analytics;
...
const analytics = new ga_cjs_1("UA-131336352-5");
const clientID = uuid();
function trackEvent() {
    analytics.send({
        category: "vscode",
